...
GameObject* GOMove::ScaleGameObject(Player* player, float scale, ObjectGuid::LowType lowguid)
{
    if (!player)
        return nullptr;
    ...
    ChatHandler(player->GetSession()).PSendSysMessage("Object scale set to: %.2f", scale);
    return object;
}
